...
export const generatePDFWithValue = async (tableData: ItemType[]) => {
...
page.drawLine({start: { x: 365, y: 102 }, end: { x: 365, y: 43 }, thickness: 1, color: rgb(0, 0, 0)});
  // Add tableData rows into PDF
  tableData.forEach((entry, index) => {
    const { stock_property_no, unit, item_description, quantity, unit_cost, total_cost } = entry;

    const yPosition = 570 - index * 20;  // Adjust y-coordinate for each entry

    page.drawText(stock_property_no, { x: 35, y: yPosition, size: 11, font: timesRomanFont });
    page.drawText(unit, { x: 80, y: yPosition, size: 11, font: timesRomanFont });
    page.drawText(item_description, { x: 206, y: yPosition, size: 11, font: timesRomanFont });
    page.drawText(quantity.toString(), { x: 385, y: yPosition, size: 11, font: timesRomanFont });
    page.drawText(unit_cost.toString(), { x: 438, y: yPosition, size: 11, font: timesRomanFont });
    page.drawText(total_cost.toString(), { x: 495, y: yPosition, size: 11, font: timesRomanFont });
  });
